package me.amitshekhar.mvvm.ui.topheadline

import android.net.Uri
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import androidx.browser.customtabs.CustomTabsIntent
import androidx.recyclerview.widget.RecyclerView
import com.bumptech.glide.Glide
import me.amitshekhar.mvvm.data.model.Children
import me.amitshekhar.mvvm.databinding.TopHeadlineItemLayoutBinding

class TopHeadlineAdapter(
    private val childrenList: ArrayList<Children>
) : RecyclerView.Adapter<TopHeadlineAdapter.DataViewHolder>() {

    class DataViewHolder(private val binding: TopHeadlineItemLayoutBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(children: Children) {
            binding.textViewTitle.text = children.data.title
            binding.textViewDescription.text = children.data.selftext
            binding.textViewSource.text = children.data.author
            if(children.data.thumbnail_url.isEmpty()) {
                binding.imageViewBanner.visibility = View.GONE
            }else {
                binding.imageViewBanner.visibility = View.VISIBLE
                Glide.with(binding.imageViewBanner.context)
                    .load(children.data.thumbnail_url)
                    .into(binding.imageViewBanner)
            }
            itemView.setOnClickListener {
                val builder = CustomTabsIntent.Builder()
                val customTabsIntent = builder.build()
                customTabsIntent.launchUrl(it.context, Uri.parse(children.data.url))
            }
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) =
        DataViewHolder(
            TopHeadlineItemLayoutBinding.inflate(
                LayoutInflater.from(parent.context),
                parent,
                false
            )
        )

    override fun getItemCount(): Int = childrenList.size

    override fun onBindViewHolder(holder: DataViewHolder, position: Int) =
        holder.bind(childrenList[position])

    fun addData(list: List<Children>) {
        childrenList.addAll(list)
    }


}